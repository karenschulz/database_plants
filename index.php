<?php
$db = new PDO('mysql:host=db; dbname=my_plant_collection', 'root' , 'password');
$query1 = $db-> prepare ("SELECT * FROM `plants`;");
$query1->setFetchMode(PDO::FETCH_ASSOC);
$query1->execute();
?>

<html lang="en-GB">

	<head>
		<title> Welcome!</title>
		<link rel="stylesheet" type="text/css" href="style.css">
	</head>

<body>
<nav class="nav_bar">
    <p class="welcome">Welcome to my own personal Plant Database</p>
    <div class="links_nav">
        <a href="form.php">Add new plants to the database</a>
    </div>
</nav>

<main>
    <h1>Hi there!</h1>
    <h1>Here you can see my stunning house plant collection:</h1>
    <div class="plants_collection">

    <?php
while ($row = $query1-> fetch()) { ?>
    <div class="plants">
            <img class="prayer_plant" src="<?php echo $row["image"]
            ?>" alt="here you can see my stunning Prayer Plant or Calathea medallion">
        <h2><?php echo $row["botanical_name"] ?></h2>
        <h3>NICKNAME:</h3>
        <?php echo $row["nickname"]
        ?>
        <h3>PLANT TYPE:</h3>
        <?php echo $row["plant_type"]
        ?>
        <h3>PLANT HEIGHT IN CM:</h3>
        <?php echo $row["plant_height"]
        ?>
        <h3>POISONOUS:</h3>
        <?php echo $row["poisonous"]
        ?>
        <h3>DATE ADDED:</h3>
        <?php echo $row["date_added"]
        ?>
        </div>
<?php
}
?>
    </div>
</main>
<div>
    <a href="form.php"><h1>Add new plants</h1></a>
</div>


<footer>&copy; Copyright. All rights reserved. Karen Schulz, February 2021</footer>
</body>

</html>


<!--<html lang="en-GB">-->
<!---->
<!--	<head>-->
<!--		<title> Welcome!</title>-->
<!--		<link rel="stylesheet" type="text/css" href="style.css">-->
<!--	</head>-->
<!---->
<!--<body>-->
<!--<nav class="nav_bar">-->
<!--    <p class="welcome">Welcome to my own personal Plant Database</p>-->
<!--    <div class="links_nav">-->
<!--        <a href="form.php">Add new plants to the database</a>-->
<!--    </div>-->
<!--</nav>-->
<!---->
<!--<main>-->
<!--    <h1>Hi!</h1>-->
<!--    <h1>Here you can see my stunning house plant collection:</h1>-->
<!---->
<!--    <div class="plants_collection">-->
<!--        <div class="plants">-->
<!--            <img class="prayer_plant" src="images/prayer_plant.jpg" alt="here you can see my stunning Prayer Plant or Calathea medallion">-->
<!--            <h2>Prayer Plant</h2>-->
<!--            <h3>BOTANICAL NAME:</h3>-->
<!--            Calathea medallion-->
<!--            <h3>NICKNAME:</h3>-->
<!--            Prayer plant-->
<!--            <h3>PLANT TYPE:</h3>-->
<!--            Perennial, indoor-->
<!--            <h3>PLANT HEIGHT:</h3>-->
<!--            76.3 cm-->
<!--            <h3>POISENOUS:</h3>-->
<!--            No-->
<!--        </div>-->
<!---->
<!--        <div class="plants">-->
<!--            <img class="elephant_ear" src="images/elephant-ear-plant.jpg" alt="beautiful elephant ear plant with a blue background">-->
<!--            <h2>Elephant Ear Plant</h2>-->
<!--            <h3>BOTANICAL NAME:</h3>-->
<!--            Alocasia zebrina-->
<!--            <h3>NICKNAME:</h3>-->
<!--            Elephant ear plant-->
<!--            <h3>PLANT TYPE:</h3>-->
<!--            Perennial, indoor-->
<!--            <h3>PLANT HEIGHT:</h3>-->
<!--            55.5 cm-->
<!--            <h3>POISENOUS:</h3>-->
<!--            Yes-->
<!--        </div>-->
<!---->
<!--        <div class="plants">-->
<!--            <img class="spider_plant" src="images/curly-spider-plant.jpg" alt="curly spider plant in a grey pot">-->
<!--            <h2>Curly Spider Plant</h2>-->
<!--            <h3>BOTANICAL NAME:</h3>-->
<!--            Chlorophytum comosum-->
<!--            <h3>NICKNAME:</h3>-->
<!--            Curly spider plant; Variegated spider plant; Airplane plant-->
<!--            <h3>PLANT TYPE:</h3>-->
<!--            Perennial indoor-->
<!--            <h3>PLANT HEIGHT:</h3>-->
<!--            30 cm-->
<!--            <h3>POISENOUS:</h3>-->
<!--            No-->
<!--        </div>-->
<!---->
<!--        <div class="plants">-->
<!--            <img class="ivy" src="images/ivy.jpg" alt="english ivy in a grey pot with white background">-->
<!--            <h2>English Ivy Plant</h2>-->
<!--            <h3>BOTANICAL NAME:</h3>-->
<!--            Hedera helix-->
<!--            <h3>NICKNAME:</h3>-->
<!--            English ivy; Common ivy; European ivy-->
<!--            <h3>PLANT TYPE:</h3>-->
<!--            Evergreen climber-->
<!--            <h3>PLANT HEIGHT:</h3>-->
<!--            34.5-->
<!--            <h3>POISENOUS:</h3>-->
<!--            Yes-->
<!--        </div>-->
<!---->
<!--        <div class="plants">-->
<!--            <img class="snake_plant" src="images/snake-plant.jpg" alt="snake plant also called mother in laws tongues in a dark grey pot">-->
<!--            <h2>Snake Plant</h2>-->
<!--            <h3>BOTANICAL NAME:</h3>-->
<!--            Sansevieria trifasciata-->
<!--            <h3>NICKNAME:</h3>-->
<!--            Snake plant; Mother-in-Laws tongue; Vipers bowst-->
<!--            <h3>PLANT TYPE:</h3>-->
<!--            Perennial, indoor-->
<!--            <h3>PLANT HEIGHT:</h3>-->
<!--            64.3 cm-->
<!--            <h3>POISENOUS:</h3>-->
<!--            Yes-->
<!--        </div>-->
<!---->
<!--        <div class="plants">-->
<!--            <img class="cheese_plant" src="images/cheeseplant.jpg" alt="cheese plant in a traditional plastic pot">-->
<!--            <h2>Cheese Plant</h2>-->
<!--            <h3>BOTANICAL NAME:</h3>-->
<!--            Monstera deliciosa-->
<!--            <h3>NICKNAME:</h3>-->
<!--            Cheese plant; Ceriman; Custard plant; Indian ivy; Fruit salad plant; Mexican breadfruit-->
<!--            <h3>PLANT TYPE:</h3>-->
<!--            Evergreen shrub, indoor-->
<!--            <h3>PLANT HEIGHT:</h3>-->
<!--            76.3cm-->
<!--            <h3>POISENOUS:</h3>-->
<!--            yes-->
<!--        </div>-->
<!---->
<!--</div>-->
<!--</main>-->
<!--        <div>-->
<!--            <a href="form.php"><h1>Add new plants</h1></a>-->
<!--        </div>-->
<!---->
<!---->
<!--<footer>&copy; Copyright. All rights reserved. Karen Schulz, February 2021</footer>-->
<!--</body>-->
<!---->
<!--</html>-->
<!---->
